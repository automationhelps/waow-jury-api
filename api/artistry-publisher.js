...
function getPageHTML() {
  return [
    ...
    '  background: #f5f1ea; color: #2f261f; margin: 0; padding: 0; font-size: 18px; line-height: 1.6; }',
    ...
    '.instructions { background: #fff; border-left: 6px solid #4a2e1a;',
    ...
    '.toolbar input[type="text"] { flex: 1; min-width: 200px; padding: 0.7em 1em; font-size: 1em;',
    '  border: 2px solid #d4cabc; border-radius: 4px; background: #fff; }',
    '.toolbar button { padding: 0.7em 1.3em; font-size: 1em;',
    '  border: none; background: #4a2e1a; color: #fff; border-radius: 4px; cursor: pointer; font-weight: 500; }',
    ...
    '.tab-count { display: inline-block; background: #e5dccd; color: #4a2e1a;',
    ...
    '.card { background: #fff; border-radius: 6px; padding: 1.5em 1.75em; margin-bottom: 1.5em;',
    ...
    '.card-header img.headshot { width: 100px; height: 120px; border-radius: 8px;',
    ...
    '.badge { display: inline-block; background: #faf8f5; color: #332f2b;',
    ...
    '.works-thumbs .work-thumb img { width: 64px; height: 64px; object-fit: cover;',
    '  border-radius: 4px; border: 1px solid #e5dccd; display: block; }',
    '.works-thumbs .work-thumb span { display: block; font-size: 0.65em; color: #6a5a48;',
    '  max-width: 64px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }',
    '.card .actions { display: flex; gap: 0.6em; flex-wrap: wrap; margin-top: 0.75em; align-items: center; }',
    '.card button { padding: 0.65em 1.1em; font-size: 0.95em;',
    ...
    '.preview-frame { margin-top: 0.75em; padding: 1em;',
    '  background: #f3f0eb; border-radius: 4px; border: 1px dashed #d4cabc; overflow: auto; }',
    ...
    '  <p>Copy each artist profile code and paste it into a Squarespace Code block.</p>',
    '</header>',
    '',
    '<main>',
    ...
    '  var fullName     = (esc(a.firstName) + " " + esc(a.lastName)).trim();',
    '  var website      = a.website || "";',
    '  var websiteHref  = normalizeUrl(website);',
    '  var websiteDomain = website ? displayDomain(website) : "";',
    '',
    '  var pills = [a.membershipType, a.medium].filter(Boolean)',
    '    .map(function(v) { return "<div class=\\"meta-pill\\">" + esc(v) + "</div>"; }).join("");',
    '',
    '  var bioParagraphs = (a.biography || "").split(/\\n\\s*\\n/).filter(function(p){ return p.trim(); })',
    '    .map(function(p){ return "<p>" + esc(p).replace(/\\n/g,"<br>") + "</p>"; }).join("");',
    '',
    '  var statParagraphs = (a.statement || "").split(/\\n\\s*\\n/).filter(function(p){ return p.trim(); })',
    '    .map(function(p){ return "<p>" + esc(p).replace(/\\n/g,"<br>") + "</p>"; }).join("");',
    '',
    '  var workArticles = (Array.isArray(a.works) ? a.works : []).map(function(w) {',
    '    var altText = esc(w.title ? w.title + ", artwork by " + fullName : "Artwork by " + fullName);',
    '    return "<article class=\\"work-card\\">" +',
    '      "<img src=\\"" + w.image + "\\" alt=\\"" + altText + "\\" onclick=\\"openLightbox(\'"+w.image+"\',\'"+altText+"\')\\">" +',
    ...
    '    return "<div class=\\"detail-row\\"><div class=\\"detail-label\\">" + row.label + "</div>" +',
    '           "<div class=\\"detail-value\\">" + row.value + "</div></div>";',
    '  }).join("");',
    '',
    '  var headshotSrc = a.headshot || "";',
    '',
    ...
    '    "    <div class=\\"artist-meta\\">" + pills + "</div>\\n" +',
    '    (bioParagraphs ? "    <div class=\\"artist-summary\\">" + bioParagraphs + "</div>\\n" : "") +',
    ...
    '      "<div class=\\"artist-bio\\">" + statParagraphs + "</div>\\n" +',
    '      "</div>\\n</section>\\n\\n" : "") +',
    '    "<section class=\\"content-grid\\">\\n" +',
    '    "  <div class=\\"main-panel\\"><div class=\\"section-title\\">Artist Statement</div>" +',
    '    "<div class=\\"statement-text\\">" + (bioParagraphs || "<p></p>") + "</div></div>\\n" +',
    '    "  <div class=\\"side-panel\\"><div class=\\"section-title\\">Artist Details</div>" +',
    '    "<div class=\\"details-list\\">" + detailRows + "</div></div>\\n" +',
    '    "</section>\\n\\n" +',
    '    (workArticles ?',
    '      "<section class=\\"gallery-section\\">\\n" +',
    '      "<div class=\\"gallery-title-wrap\\"><div class=\\"section-title\\">Selected Artwork</div></div>\\n" +',
    '      "<div class=\\"gallery-grid\\">" + workArticles + "</div>\\n" +',
    '      "</section>\\n" : "") +',
    '    "</div>\\n\\n" +',
    ...
    '',
    '  listEl.innerHTML = artists.map(function(a, i) {',
    '    var fullName = (esc(a.firstName) + " " + esc(a.lastName)).trim() || "(No name)";',
    '    var headshotEl = a.headshot',
    '      ? "<img class=\\"headshot\\" src=\\"" + a.headshot + "\\" alt=\\"" + fullName + "\\">"',
    '      : "";',
    '    var badges = [a.medium, a.membershipType].filter(Boolean)',
    ...
    '    var websiteLink = a.website',
    '      ? "<a href=\\"" + esc(normalizeUrl(a.website)) + "\\" target=\\"_blank\\" rel=\\"noopener\\">" + esc(a.website) + "</a>"',
    '      : "—";',
    '    var workThumbs = (Array.isArray(a.works) && a.works.length)',
    '      ? a.works.slice(0,6).map(function(w) {',
    '          return "<div class=\\"work-thumb\\">" +',
    ...
    '  var a = tab === "queue" ? queueArtists[i] : archiveArtists[i];',
    '  var snippet = buildSnippet(a);',
    ...
    '    setTimeout(function() { btn.textContent = orig; btn.classList.replace("copied","primary"); }, 2500);',
    ...
    '  try {',
    '    var resp = await fetch("/api/artistry-tag", {',
    '      method: "POST",',
    ...
    '    if (action === "publish") {',
    '      var artist = queueArtists.splice(i, 1)[0];',
    '      archiveArtists.unshift(artist);',
    '    } else {',
    '      var artist2 = archiveArtists.splice(i, 1)[0];',
    '      queueArtists.unshift(artist2);',
    '    }',
    '    updateCounts();',
    '    applySearch();',
    '  } catch(e) {',
    ...
    '    queueArtists   = (results[0].ok ? results[0].artists : []) || [];',
    '    archiveArtists = (results[1].ok ? results[1].artists : []) || [];',
    '    updateCounts();',
    '    applySearch();',
    '  } catch(e) {',
    ...
    '  var source = currentTab === "queue" ? queueArtists : archiveArtists;',
    '  var filtered = q ? source.filter(function(a) {',
