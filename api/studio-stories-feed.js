...
module.exports = async (req, res) => {
  ...
  try {
    // 1) Search for tagged contacts (returns slim records)
    const searchBody = {
      locationId,
      pageLimit: 100,
      filters: [
    ...
    // 2) Fetch each contact by ID to get full customFields (incl. file uploads)
    const stories = [];
    ...
    for (const c of slim) {
      ...
      // Collect ALL uploaded images as proxied URLs
      const images = [];
